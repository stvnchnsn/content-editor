...
const CsvApp = lazy(() => import('./CsvApp'));
...
export default function App() {
  ...
  const [editorType, setEditorType] = useState<EditorType>(() => {
    ...
    if (file) {
      ...
      if (file.endsWith('.csv') || file.endsWith('.tsv')) return 'csv';
      return 'markdown';
    }
    return 'json';
  });
    ...
    <div className={styles.app}>
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          <EditorTypeSwitcher activeType={editorType} onTypeChange={setEditorType} />
          <h1 className={styles.title}>{EDITOR_TITLES[editorType]}</h1>
        </div>
        <button
